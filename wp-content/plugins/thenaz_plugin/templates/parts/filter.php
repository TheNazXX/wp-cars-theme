<div class="filter-form d-flex justify-content-center">
  ...
  <form method="post" action="<?php get_post_type_archive_link('property')?>">
    <div class="input-group mb-3">
      ...
      <input type="number" class="form-control" placeholder="Maximum Price" name="thenaz_price" min="0"
        value="<?php if(!empty($_POST['thenaz_price'])){echo esc_attr($_POST['thenaz_price']);}?>">
    </div>


    <select name="thenaz_type">
    ...
    <select name="thenaz_agent">

      <option value="" selected>Choose Agent</option>

      <?php $agents = get_posts(['post_type' => 'agent', 'numberspost' => -1]);?>

      <?php foreach($agents as $agent) {?>
      <option value="<?php echo $agent->ID?>"
        <?php if(isset($_POST['thenaz_agent']) && selected($agent->ID, $_POST['thenaz_agent'], false)){echo 'selected';};?>>
        <?php esc_html_e($agent->post_title); ?> </option>
      <?php }?>
    ...
    <select name="thenaz_location">
      ...
      <?php  echo $thenaz_plugin->get_terms_hierarchicle('location', $_POST['thenaz_location'])?>
    </select>

    <select name="thenaz_type-house">
      <option value="">Select Type</option>
      <?php  echo $thenaz_plugin->get_terms_hierarchicle('property-type', $_POST['thenaz_type-house'])?>
    </select>
